...
module.exports.createShipment = async (req, res) => {
  try {
    const shipmentId = uuid4();
    const shipmentData = req.body;
    shipmentData.shipmentId = shipmentId;
    // console.log(req.body);
    const { error } = shipmentSchema.validate(shipmentData);
    if (error)
      return res
        .status(400)
        .json({ msg: `error in validation ${error.details[0].message}` });
    const containerId = shipmentData.containerId;
    const existingShipment = await Shipment.findOne({ containerId });
    if (existingShipment) {
      return res.status(400).json({ msg: "Already a Shipment with current ContainerId" });
    }
    const user = await User.findById(shipmentData.userId);
    if (!user) {
      return res.status(404).json({ msg: "User not found" });
    }
    shipmentData.owner = user.username;
  ...
  try {
    ...
    if (error)
      return res
        .status(400)
        .json({
          error: `error in validation date ${error.details[0].message}`,
        });
...
module.exports.updateShipmentStatus = async (req, res) => {
  try {
    ...
    const statusSchema = joi.object({
      status: joi
        .string()
        .valid("Delivered", "In Transit", "Pending", "Cancelled")
        .required(),
    });
    ...
    if (error)
      return res
        .status(400)
        .json({
          error: `error in validation status ${error.details[0].message}`,
        });
    ...
    if (!shipment)
      return res
        .status(404)
        .json({ error: "Shipment not found during status update" });
...
module.exports.getShipmentETA = async (req, res) => {
  try {
    ...
    if (!shipment)
      return res
        .status(404)
        .json({ error: "shipment not found during eta shipment" });
...
module.exports.deleteShipment = async (req, res) => {
  try {
    ...
    if (!shipment)
      return res
        .status(404)
        .json({ error: "Shipment not found during deletion" });
